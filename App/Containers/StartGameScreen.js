...
import ListItem from '../Components/ListItemBookings'
...
import sort from "sort-json-array";
...
export default class StartGameScreen extends React.Component {
  ...
  constructor(props) {
    ...
    this.bookingsRef = firebaseApp.database().ref().child('/Bookings');
    this.userRef = firebaseApp.database().ref().child('/users');
    this.filterByDate = this.filterByDate.bind(this)
    //this.filterByDate2 = this.filterByDate2.bind(this)
  ...
  filterByDate(bookinfgsRef) {
    this.setState({
      InitialbookingsArray: []
    }, function () {

      var bookingsArray = this.state.InitialbookingsArray
      //alert(this.state.initialPosition.latitude + "," + this.state.initialPosition.longitude)
      this.bookingsRef.on('value', (dataSnapshot) => {
        dataSnapshot.forEach((child) => {

          let filterDate = moment(this.state.chosenDate).format('MM/DD/YYYY')

          //  Alert.alert('Selected Date', filterDate.toString() + ' Child data '+  child.val().date.split(" ")[0])
          if (filterDate === child.val().date.split(" ")[0]) {
            bookingsArray.push({
     ...
     let sorted_booking_array = sort(bookingsArray,'date', 'asc')
      this.setState({

        InitialbookingsArray: sorted_booking_array

      }, function () {

        setTimeout(() => {
          this.setState({
            dataSource: this.state.dataSource.cloneWithRows(this.state.InitialbookingsArray)
 ...
 getUserData(userId){
    ...
    this.userRef.on('value', (snapshot) => {
      ...
      snapshot.forEach((child) => {
       // Alert.alert('User id',child.key['email'])
        if(userId === child.val().userId){
          user.push({
            Name:child.val().Name,
            userId:child.val().userId,
          })
          ...
          this.setState({
           ...
           /// Alert.alert('User data',JSON.stringify(this.state.userdata))
          })
        }
      });
    });
  ...
  bookingsArray.sort()
    this.setState({
      ...
      InitialbookingsArray: bookingsArray
    }, function () {
      setTimeout(() => {
        this.setState({
          dataSource: this.state.dataSource.cloneWithRows(this.state.InitialbookingsArray)
  ...
  filterByDate2(bookinfgsRef) {
    this.setState({
      ...
      this.bookingsRef.on('value', (dataSnapshot) => {
        dataSnapshot.forEach((child) => {
          let todaydate = moment(new Date().toDateString)
          let filterDate = moment(todaydate.format('MM/DD/YYYY'))
  ...
  async getUserId(){
    try {
      ...
      if (userId !== null){
        ...
        //Alert.alert('User id from store',value)
      }else{
        Alert.alert('JSON userdata','No data')
      }
    } catch (error) {
      Alert.alert('Async error',JSON.stringify(error))
    }
  }
  componentDidMount() {

  let today = moment(new Date()).format('MM/DD/YYYY')
    this.listenForTasks(this.bookingsRef);
    //this.setDate2;
   //this.listenForTasks2(this.bookingsRef);
   this.filterByDate2();
   //renderItem();
  }
  componentWillMount() {
  ...
  handlePressBookDetails = () => {
    let formattedDate = moment(this.state.chosenDate)
    let endTime = moment(formattedDate).add(this.state.hours, 'hours')
    // Alert.alert('Date in state',this.state.chosenDate +'Moment End Date ' + moment(formattedDate).add(this.state.hours,'hours').format('MM-DD-YYYY h:mm A'))
      ...
      let d = params.lights;
      let e = params.size;
      let f = this.state.date;
      let g = this.state.hours;



      // Alert.alert("Booking Details", "Pitch Name: " + a +"\n" + "Date/Time: " + f +"\n"+ "Number Of Hours: "+
      // g +"\n" + "End Time: " + h + ":"+ k + "\n " + "new : " + j + ":" + k);

      this.getFirebaseRef().child('Bookings').push({
        title: params.title,
        address: params.address,
        userId: this.state.userdata[0].userId,
        name:this.state.userdata[0].Name,
        date: formattedDate.format('MM/DD/YYYY h:mm A').toString(),
        starttime: '10:45',
        endtime: endTime.format('MM/DD/YYYY h:mm A').toString()
      ...
      let shareOptions = {
        title: "Friend Request to Join",
        message: "Hello Friend, Join Me for a Game of Soccer at a Mettle Sports Pitch",
        url: "http://www.mettlesports.com/",
      ...
      AlertIOS.alert(
        ...
        'Do you want to invite your friends to the Game?',
        [
          {
            text: 'No',
  ...
  setDate(newDate) {
    this.setState({
      ...
      this.filterByDate(this.bookingsRef);
  ...
  render() {
    ...
    return (
      ...
      <ScrollView contentContainerStyle={{ justifyContent: 'center' }} style={[Styles.container]}
        ...
        <View>
          <View>
            <Text style={Styles.normalTextStyle3}>
            {"\n"}Selected Field: {this.state.title}{"\n"}
            </Text>
            ...
            {/* <DatePicker
              ...
              minDate="YYYY-MM-DD hh:mm A"
              maxDate="2021-06-01 hh:mm A"
            ...
            {/*<Text style={Styles.normalTextStyle2}>
              {"\n"}Selected Date & Time: {this.state.newdate.toString()}{}
          </Text>*/}
            <Dropdown style={Styles.dropDown} onChangeText={this.handleChangeItem}
              label='Number of Hours you want to play'
              align-items =  'center'
              data={data}
            />

            {/*<Button style={Styles.button1} onPress={this.handlePressPitchDetails} title='Pitch Details'>
              Pitch Details
        </Button>*/}
            <RoundedButton style={Styles.Button1} onPress={this.handlePressBookDetails}
              title='Confirm Stepping Inside and Feel The Power of the Pitch™'>
              Confirm Stepping Inside and Feel The Power of the Pitch™
                         </RoundedButton>
            <Text style={Styles.normalTextStyle2}>
              {}Players Playing on the Selected Date{}{"\n"}
          </Text>
          ...
          </View>
          {
            this.state.dataSource && (
              <ListView
                ...
                //renderRow={this.renderItem2.bind(this)}
